 ...
 *
 *	- long until(Temporal end, ChronoUnit u): 차이 계산 ChornoUnit 뒤에오는 기준으로 계산(MONTHS,YEARS...) enum 이다
 *  		 
 */
public class Ex06_LocalDateTime {
	public static void main(String[] args) {
		...
		//startDateTime이 endDateTime 보다 이전인가?
		if(startDateTime.isBefore(endDateTime)) {
			System.out.println("진행 중입니다.");
		//startDateTime이 endDateTime 보다 같은가?	
		} else if(startDateTime.isEqual(endDateTime)) {
			System.out.println("종료합니다.");
		//startDateTime이 endDateTime 보다 이후인가?	
		...
		System.out.println("남은 해: " + remainYear);//YEAR는 ‘연도 숫자 차이’가 아님,365일(또는 윤년) 단위가 꽉 차야 1
		System.out.println("남은 월: " + remainMonth);//날짜가 같은 날에 도달해야 1
